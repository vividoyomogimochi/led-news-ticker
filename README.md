  ...
  "sources": [
    {
      "id": "my-rss",
      "label": "My RSS",
      "params": { "type": "rss", "url": "https://example.com/feed.xml", "noproxy": "1" }
    }
  ],
  "displays": [
    {
...
| `label` | UI に表示される名前 |
| `params` | URL クエリパラメータとしてティッカーに渡される値。RSS ソースで `"noproxy": "1"` を指定するとプロキシを経由せず直接フェッチする |
...
| `SseSource` | SSE（Server-Sent Events）経由でリアルタイムに受信 |

### 複数ソース

クエリパラメータに番号を付けて複数のソースを同時に登録できます。1 つ目はサフィックスなし、2 つ目以降は `type2`/`url2`、`type3`/`url3`... のように指定します。

```
/?type=rss&url=https://feed1.example.com/rss
  &type2=rss&url2=https://feed2.example.com/rss
  &type3=sse&url3=https://stream.example.com/events
```

各ソースは Scheduler 内で独立したサブキューを持ち、ラウンドロビンで交互に表示されます。設定画面の Customize タブからも「+ ソースを追加」でソースを追加・削除できます。

### CORS プロキシ

RSS フィードの多くは CORS ヘッダーを返さないため、Cloudflare Pages Function（`/proxy?url=`）経由でフェッチします。プロキシはオープンプロキシ化を防ぐため、レスポンスの Content-Type が XML / RSS / Atom / text/plain のいずれかでない場合はリクエストを拒否します。

フィード側が `Access-Control-Allow-Origin: *` を返す場合はプロキシ不要です。設定画面の「CORS プロキシを使用する」チェックボックスをオフにするか、クエリパラメータに `noproxy=1` を付けると直接フェッチに切り替わります。
